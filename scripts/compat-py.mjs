...
if (!result) {
  ...
  if (!result.ok && /401|unauthorized|invalid_token/i.test(String(result.error?.cause ?? ""))) result.error = { code: "AUTH_REQUIRED", cause: result.error?.cause ?? "401", fix: "Add an Authorization header or OAuth refresh credentials to the enrollment." };
